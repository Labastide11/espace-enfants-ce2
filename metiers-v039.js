// Espace Enfants CE2 — Mon métier — V0.39
...
// Élèves qui ne participent jamais à la rotation des métiers.
...
const JOBS_EXCLUDED = new Set(["ritej", "anis", "rayan"]);
const isEligibleForJobs = name => !JOBS_EXCLUDED.has(normalizeStudentName(name));
const ELIGIBLE_STUDENTS = ALL_STUDENTS.filter(isEligibleForJobs);
...
const NEW_HISTORY_KEY = "nino_metiers_history_v6";
const NEW_CURRENT_PREFIX = "nino_metiers_week_v6_";
...
function makeNewDraw(weekKey,history){
  if(ELIGIBLE_STUDENTS.length!==PRIMARY_SLOT_COUNT){
    console.warn(
      `[Métiers] ${ELIGIBLE_STUDENTS.length} élèves éligibles pour ${PRIMARY_SLOT_COUNT} places principales. ` +
      `Ritej, Anis et Rayan restent exclus dans tous les cas.`
    );
  }
...
function drawContainsExcludedStudent(draw){
  return !!draw?.assignments?.some(a =>
    (a.students || []).some(n => !isEligibleForJobs(n))
  );
}
...
function loadNewDraw(weekKey){
  ...
  // Si un ancien aperçu contient Ritej, Anis ou Rayan,
  // il est automatiquement supprimé puis recalculé.
  if(d?.assignments && drawContainsExcludedStudent(d)){
    d=null;
    delete h[weekKey];
  }
  ...
  return `<article class="assignment-card${repl?" assignment-card--replacement-role":""}${mission?" assignment-card--mission":""}">
    <button class="assignment-toggle" type="button" aria-expanded="false">
      <div class="assignment-job">
        ...
        ${mission?'<span class="replacement-badge">⭐ mission en plus</span>':""}
