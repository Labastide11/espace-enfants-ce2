// Espace Enfants CE2 — Mon métier — V0.35
...
document.querySelector("#draw-status").textContent=isNew
...
function renderStats(){
  ...
  const records=Object.values(h).filter(r=>r?.assignments);
  const s=newStats(records);
  statsSummary.textContent=`${records.length} semaine(s) comptabilisée(s) avec les nouvelles règles.`;
