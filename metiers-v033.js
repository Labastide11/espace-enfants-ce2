// Espace Enfants CE2 — Mon métier — V0.33
const ELEVES = Array.isArray(window.NINO_ELEVES) ? window.NINO_ELEVES : [];
const STUDENTS = ELEVES.map(e => e.prenom);

const JOBS = [
  {icon:"📦",name:"Distributeur",banner:"assets/metiers/distributeur.png?v=032",detail:"Je distribue et je ramasse les cahiers, fiches et documents.",rule:"Je fais vite, calmement et sans jouer.",fixedPair:true},
  {icon:"🚶",name:"Chef de rang",banner:"assets/metiers/chef-de-rang.png",detail:"Je conduis le rang calmement et en sécurité.",rule:"Je marche et j’attends le groupe."},
  {icon:"📚",name:"Bibliothécaire",banner:"assets/metiers/bibliothecaire.png",detail:"Je range et je prends soin des livres.",rule:"Je manipule les livres avec soin."},
  {icon:"✉️",name:"Facteur",banner:"assets/metiers/facteur.png",detail:"Je porte les messages ou documents dans l’école.",rule:"Je vais directement à l’endroit demandé puis je reviens.",fixedPair:true},
  {icon:"🎤",name:"Animateur du Quoi de neuf",banner:"assets/metiers/animateur-quoi-de-neuf.png",detail:"J’annonce les passages et je distribue la parole.",rule:"Je veille à ce que chacun puisse parler."},
  {icon:"🧹",name:"Agent d’entretien",banner:"assets/metiers/agent-entretien.png",detail:"J’aide à garder la classe propre et rangée.",rule:"Je n’utilise que le matériel prévu."},
  {icon:"📝",name:"Écrivain",banner:"assets/metiers/ecrivain.png",detail:"J’écris la date au tableau.",rule:"Je vérifie la date avant de l’écrire."},
  {icon:"🔐",name:"Gardien",banner:"assets/metiers/gardien.png",detail:"Avec l’accord du maître, je ferme à clé les portes nécessaires.",rule:"Je ne ferme une porte que si le maître le demande."},
  {icon:"🦜",name:"Perroquet",banner:"assets/metiers/perroquet.png?v=033",detail:"Je reformule une consigne importante avec mes mots.",rule:"Je n’ajoute pas de nouvelle consigne."},
  {icon:"🤫",name:"Gardien du calme",banner:"assets/metiers/gardien-du-calme.png",detail:"J’aide la classe à rester calme.",rule:"Je rappelle gentiment la règle sans commander."}
];
...
current.innerHTML=d.assignments.map((a,i)=>{
  ...
  return `<article class="assignment-card${repl?" assignment-card--replacement-role":""}">
    <button class="assignment-toggle" type="button" aria-expanded="false">
      <div class="assignment-job"><img class="assignment-job-banner" src="${j.banner}" alt="${esc(j.name)}">${repl?'<span class="replacement-badge">+ remplaçant</span>':""}<span class="assignment-chevron">⌄</span></div>
...
grid.innerHTML=JOBS.map((j,i)=>`<button class="job-card job-card--banner" data-job="${i}" type="button"><img class="job-card-banner" src="${j.banner}" alt="${esc(j.name)}"><span class="job-pair">👥 ${d.assignments.find(a=>a.job===j.name).students.join(" + ")}</span></button>`).join("");
